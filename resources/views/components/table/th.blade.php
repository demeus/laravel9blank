...
<th {{ $attributes->merge(['class' => 'border-b-2 dark:border-dark-5 whitespace-nowrap'])->only('class') }}>
    @unless ($sortable)
        <span class="text-xs font-medium text-gray-500 uppercase">{{ $slot }}</span>
    @else
        <button {{ $attributes->except('class') }} class="flex items-center space-x-1 text-left text-xs font-medium text-gray-500 uppercase group focus:outline-none focus:underline">
            <span>{{ $slot }}</span>
